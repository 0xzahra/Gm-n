...
export const NeonButton: React.FC<NeonButtonProps> = ({ variant = "primary", children, className = "", flashing = false, ...props }) => {
  ...
  const variants = {
    ...
    primary: "bg-neo-green text-neo-black hover:bg-white hover:text-neo-green shadow-neon",
    // Secondary: Transparent with Green Border. Hover: Green background tint.
    secondary: "bg-neo-green/5 text-neo-green border border-neo-green/30 hover:border-neo-green hover:bg-neo-green/10 hover:shadow-neon",
    // Ghost: Minimalist
    ghost: "bg-transparent text-neo-green/70 hover:text-neo-green hover:bg-neo-green/5",
    // Danger: Red tones
    danger: "bg-red-900/20 text-red-500 border border-red-500/50 hover:bg-red-500/20",
  };

  // Using !important to ensure override during loading state
  const flashClass = flashing ? "!bg-white !text-neo-green !shadow-[0_0_20px_#00FF41] animate-pulse-fast cursor-wait" : "";
